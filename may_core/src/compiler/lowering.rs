...
impl<'a> SemanticLowering<'a> {
    ...
    pub fn lower_expr(&self, expr: Expr) -> Result<Expr, LoweringError> {
        match expr {
                ...
                for model in self.state.models.values() {
                    if let Some(e) = model.entities.iter().find(|e| e.name == entity) {
                        if let Some(m) = e.measures.iter().find(|m| m.name == measure) {
                            return Ok(m.agg.to_expr(Expr::Column(ColumnIdent(format!("{}.{}", e.table, m.sql)))));
                        }
            ...
            Expr::Function { name, args } => {
                let lowered_args = args.into_iter()
                    .map(|arg| self.lower_expr(arg))
                    .collect::<Result<Vec<_>, _>>()?;
                Ok(Expr::Function { name, args: lowered_args })
            }
            other => Ok(other),
        }
    }
    ...
    pub fn lower_node(&self, node: SqlNode) -> Result<SqlNode, LoweringError> {
        match node {
            SqlNode::Query { ctes, select, from, r#where, group_by, having } => {
                let ctes = ctes.map(|c| c.into_iter().map(|n| self.lower_node(n)).collect::<Result<Vec<_>, _>>()).transpose()?;
                let select = Box::new(self.lower_node(*select)?);
                let from = Box::new(self.lower_node(*from)?);
                let r#where = r#where.map(|n| self.lower_node(*n)).transpose()?.map(Box::new);
                let group_by = group_by.map(|n| self.lower_node(*n)).transpose()?.map(Box::new);
                let having = having.map(|n| self.lower_node(*n)).transpose()?.map(Box::new);
                Ok(SqlNode::Query { ctes, select, from, r#where, group_by, having })
            }
            ...
            SqlNode::Join { join_type, relation, on } => {
                let relation = Box::new(self.lower_node(*relation)?);
                let on = self.lower_expr(on)?;
                Ok(SqlNode::Join { join_type, relation, on })
            }
